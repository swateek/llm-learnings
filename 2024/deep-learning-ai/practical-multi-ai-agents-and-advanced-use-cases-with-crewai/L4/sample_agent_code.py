...
# Priority Levels
priority_df_melted = priority_df.melt(
    id_vars=["Issue Type"], var_name="Priority", value_name="Count"
)
...
sns.lineplot(
    x="Agent ID",
    y="Average Resolution Time (mins)",
    data=agent_performance_df,
    marker="o",
)
...
ax1.bar(
    agent_performance_df["Agent ID"],
    agent_performance_df["Average Resolution Time (mins)"],
    color=color,
    alpha=0.6,
)
...
ax2.plot(
    agent_performance_df["Agent ID"],
    agent_performance_df["Average Satisfaction Rating"],
    color=color,
    marker="o",
)
